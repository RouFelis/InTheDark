Shader "Hidden/Shader/NTSCEncode_RLPRO"
{
	...
	float4 CompositeSample(float2 UV) 
	{
		...
		float3 Texel0 = SAMPLE_TEXTURE2D_X(_MainTex,s_point_clamp_sampler, C0  ).rgb;
		float3 Texel1 = SAMPLE_TEXTURE2D_X(_MainTex,s_point_clamp_sampler, C1  ).rgb;
		float3 Texel2 = SAMPLE_TEXTURE2D_X(_MainTex,s_point_clamp_sampler, C2  ).rgb;
		float3 Texel3 = SAMPLE_TEXTURE2D_X(_MainTex,s_point_clamp_sampler, C3  ).rgb;
		...
		float4 Frag0(Varyings i) : SV_Target
	{
		...
		half4 col1 = SAMPLE_TEXTURE2D_X(_MainTex,s_point_clamp_sampler, positionSS);
		...
		return lerp(col1, NTSCCodec(UV), _Intensity);
