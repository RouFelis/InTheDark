...
Shader "Hidden/Shader/Bleed_RLPRO_HDRP"
{
	...
	{
		float2 positionSS = p ;
		float3 outColor = SAMPLE_TEXTURE2D_X(_MainTex,s_linear_clamp_sampler, positionSS).xyz;
	...
	float4 CustomPostProcessNTSCOld3Phase(Varyings input) : SV_Target
	{
		...
		half4 col23 = SAMPLE_TEXTURE2D_X(_MainTex,s_linear_clamp_sampler, fixCoord);
