Shader "Hidden/Shader/JitterEffect_RLPRO"
{
	Properties
    {
	...
	float3 yiqDist(float2 uv, float m, float t) {
		...
		signal.x = rgb2yiq(SAMPLE_TEXTURE2D_X_LOD(_MainTex, s_point_clamp_sampler, float2(offsetX.r, uv.y) , 0.0).rgb).x;
		signal.y = rgb2yiq(SAMPLE_TEXTURE2D_X_LOD(_MainTex, s_point_clamp_sampler, float2(offsetX.g, uv.y) , 0.0).rgb).y;
		signal.z = rgb2yiq(SAMPLE_TEXTURE2D_X_LOD(_MainTex, s_point_clamp_sampler, float2(offsetX.b, uv.y) , 0.0).rgb).z;
		return signal;
	}
#endif
#if VHS_TWITCH_V_ON	
	...
	float4 Frag0(Varyings i) : SV_Target
	{
		...
		col = 
			SAMPLE_TEXTURE2D_X_LOD(_MainTex, s_point_clamp_sampler, p , 0.0).rgb;
		signal = rgb2yiq(col);
#endif
		...
		return lerp(colIn,half4(col, 1),fade);
	}
