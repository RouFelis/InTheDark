Shader "Hidden/Shader/TV_RLPRO_HDRP"
{
	...
	float3 Fetch(float2 pos, float2 off)
	{
		...
		return SAMPLE_TEXTURE2D_X_LOD(_MainTex, s_linear_clamp_sampler, pos,0.0).rgb;
	}
    ...
    float4 CustomPostProcessTVWarp(Varyings input) : SV_Target
    {
        ...
        float4 outColor = SAMPLE_TEXTURE2D_X(_MainTex,s_linear_clamp_sampler, UV);			
		res = _ScreenSize.xy / resScale;
		float2 fragCoord = UV * _ScreenSize.xy ;
		float4 fragColor = float4(0, 0, 0, 1);
		float2 pos = Warp1(fragCoord.xy / _ScreenSize.xy);
	...
	float4 CustomPostProcessTVCubic(Varyings input) : SV_Target
	{
		...
		float2 fragCoord = UV * _ScreenSize.xy ;
		float4 fragColor = float4(0,0,0,1);
		float2 pos = Warp(fragCoord.xy  / _ScreenSize.xy );
		fragColor.rgb = Tri(pos) * Mask(fragCoord);
				if (_FadeMultiplier > 0)
		{
			#if ALPHA_CHANNEL
