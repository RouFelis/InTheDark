Shader "Hidden/Shader/OldFilmEffect_RLPRO"
{
		...
		float4 Frag0(Varyings i) : SV_Target
	{
		...
		float3 image = SAMPLE_TEXTURE2D_X(_MainTex,s_point_clamp_sampler, float2(suv.x, suv.y)).rgb;
		...
		float4 result = float4(oldImage * vI, SAMPLE_TEXTURE2D_X(_MainTex,s_point_clamp_sampler, float2(suv.x, suv.y)).a);
		...
		result = lerp(result, SAMPLE_TEXTURE2D_X(_MainTex,s_point_clamp_sampler, i.texcoord), 1 - Fade);
		return result;
	}
