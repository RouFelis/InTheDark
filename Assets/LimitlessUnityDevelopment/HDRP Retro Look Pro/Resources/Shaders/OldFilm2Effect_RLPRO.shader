Shader "Hidden/Shader/OldFilm2Effect_RLPRO" ///111
{
	Properties
    {
	...
	float4 Frag0(Varyings i) : SV_Target
	{
        ...
        float4 noise1 = SAMPLE_TEXTURE2D_X(_MainTex, s_point_clamp_sampler, uv + offset);
        float4 color = SAMPLE_TEXTURE2D_X(_MainTex, s_point_clamp_sampler, uv);
        float amount = 1* _Grain;
		float4 colour = lerp(color, noise1, amount);

		float2 vUv = i.texcoord;
		...
		return lerp(color,float4(finalColour,1),Fade);
	}
