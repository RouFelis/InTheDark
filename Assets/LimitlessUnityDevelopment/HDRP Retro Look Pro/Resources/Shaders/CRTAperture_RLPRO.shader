Shader "Hidden/Shader/CRTAperture_RLPRO"
{
	...
	float3 TEX2D(float2 c) {
		return	pow(abs(SAMPLE_TEXTURE2D_X(_MainTex,s_point_clamp_sampler, c).rgb), float3(GAMMA_INPUT, GAMMA_INPUT, GAMMA_INPUT)).xyz;
	}
	...
	float4 Frag(Varyings i) : SV_Target
	{
		...
		float3 col_glow = filter_gaussian(UV, _ScreenParams.xy);
		float3 col_soft = filter_lanczos(UV, _ScreenParams.xy, 1);
		float3 col_sharp = filter_lanczos(UV, _ScreenParams.xy, 3);
		...
		half4 colIn = SAMPLE_TEXTURE2D_X(_MainTex,s_point_clamp_sampler, UV);
		...
		return lerp(colIn,float4(col, colIn.a),fade);
	}
